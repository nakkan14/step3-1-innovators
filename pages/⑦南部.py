...
# 表示するデータを読み込み
df = pd.read_csv('物件.csv')
df_park = pd.read_csv('公園.csv')
df_school = pd.read_csv('小学校.csv')
df_nursery = pd.read_csv('保育園.csv')
df_kindergarten = pd.read_csv('幼稚園.csv')
df_lesson=pd.read_csv('習い事.csv')
df_supermarket = pd.read_csv('スーパー.csv')
df_store = pd.read_csv('コンビニ.csv')
df_hospital = pd.read_csv('病院.csv')

# 地区ブロックの設定（南部地域だけ地図表示の問題があるためブロック設定を詳細にしています）
df= df[df['地区_コード'] == 6] # ここを7にすると地図が表示されません。ですので緊急避難的に6にしています。
df_hospital = df_hospital[df_hospital['Block'] == '南部地域']
df_supermarket = df_supermarket[df_supermarket['Block'] == '南部地域']
df_kindergarten = df_kindergarten[df_kindergarten['地区_ブロック'] == '南部地域']
df_nursery = df_nursery[df_nursery['地区_ブロック'] == '南部地域']
df_school = df_school[df_school['地区_ブロック'] == '南部地域']
df_park = df_park[df_park['地区_ブロック'] == '南部地域']
df_lesson = df_lesson[df_lesson['地区_ブロック'] == '南部地域']
df_store = df_store[df_store['地区_ブロック'] == '南部地域']

# 地図の基本設定⇒locationを地区ブロックごとに調整する
m = folium.Map(
    location=[34.19353238682323, 135.1682543215429],
    tiles='OpenStreetMap',
    zoom_start=13
)
